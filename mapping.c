 ...
 *
 * Original binary tree version for LPCA written by one of the earliest MUD-OS
 * hackers.
 * - some enhancements by Truilkan@TMI
 * - rewritten for MUD-OS to use an extensible hash table implementation styled
...
INLINE_STATIC unsigned long  node_hash P1(mapping_node_t *, mn) {
...
void add_mapping_pair P3(mapping_t *, m, char *, key, long, value)
{
    svalue_t *s;
