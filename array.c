...
#endif
INLINE_STATIC long alist_cmp PROT((svalue_t *, svalue_t *));
...
/* Finish setting up an array allocated with ALLOC_ARRAY, resizing it to
   size n */
static array_t *fix_array P2(array_t *, p, unsigned int, n) {
#ifdef ARRAY_STATS
    num_arrays++;
    total_array_size += sizeof(array_t) + sizeof(svalue_t) * (n-1);
#endif
...
INLINE_STATIC array_t *resize_array P2(array_t *, p, unsigned int, n) {
#ifdef ARRAY_STATS
...
INLINE_STATIC long alist_cmp P2(svalue_t *, p1, svalue_t *, p2)
{
    long d;
